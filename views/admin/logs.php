        ...
        <div class="d-md-flex d-block align-items-center justify-content-between my-4 page-header-breadcrumb">
            <div class="d-flex align-items-center gap-3">
                ...
                <?php if (checkPermission($getUser['admin'], 'edit_logs')): ?>
                    <button type="button" class="btn btn-outline-danger btn-sm" data-bs-toggle="modal" data-bs-target="#cleanupLogsModal">
                                ...
                                <tbody>
                                    ...
                                    foreach ($listDatatable as $row): ?>
                                        <tr>
                                            ...
                                            <td><span class="text-wrap" style="max-width: 300px; display: inline-block;"><?= htmlspecialchars($row['action']); ?></span></td>
                                            ...
                                            <td><span class="text-wrap" style="max-width: 200px; display: inline-block;"><?= htmlspecialchars($row['device']); ?></span></td>
                                        </tr>
                                    <?php endforeach ?>
                                </tbody>
                            </table>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="cleanupLogsModalLabel">
                    <i class="ri-delete-bin-line text-danger me-2"></i><?= __('Dọn dẹp nhật ký hoạt động'); ?>
                </h5>
            ...
            <div class="modal-body">
                <div class="alert alert-warning border-0">
                    ...
                    <?= __('Lưu ý: Thao tác này sẽ xóa vĩnh viễn các bản ghi nhật ký và không thể hoàn tác.'); ?>
                </div>
                <div class="mb-3">
                    <label for="cleanupDays" class="form-label fw-medium"><?= __('Xóa nhật ký cũ hơn'); ?></label>
                    <div class="input-group">
                        <input type="number" class="form-control" id="cleanupDays" value="30" min="1" max="365" placeholder="30">
                ...
                <div id="cleanupPreview" class="d-none">
...
<script>
    $(document).ready(function() {
        var cleanupModal = document.getElementById('cleanupLogsModal');
        var $cleanupDays = $('#cleanupDays');
        var $cleanupPreview = $('#cleanupPreview');
        var $cleanupPreviewText = $('#cleanupPreviewText');
        var $confirmBtn = $('#confirmCleanupBtn');
        ...
        function updatePreview() {
            ...
            $.ajax({
                ...
                dataType: 'json',
                data: {
                    action: 'previewCleanupLogs',
                    days: days
                },
                success: function(resp) {
                    if (resp.status === 'success') {
                        $cleanupPreviewText.text('<?= __('Sẽ xóa'); ?> ' + resp.count + ' <?= __('bản ghi nhật ký'); ?>');
        ...
        $confirmBtn.on('click', function() {
            ...
            if (days < 1) {
                Swal.fire({
                    icon: 'warning',
                    title: '<?= __('Cảnh báo'); ?>',
                    text: '<?= __('Vui lòng nhập số ngày hợp lệ (tối thiểu 1 ngày)'); ?>'
                });
                return;
            }
            ...
            Swal.fire({
                ...
                text: '<?= __('Bạn có chắc chắn muốn xóa tất cả nhật ký cũ hơn'); ?> ' + days + ' <?= __('ngày? Thao tác này không thể hoàn tác.'); ?>',
                        ...
                        success: function(resp) {
                            if (resp.status === 'success') {
                                ...
                                Swal.fire({
                                    icon: 'error',
                                    title: '<?= __('Lỗi'); ?>',
                                    text: resp.msg || '<?= __('Có lỗi xảy ra'); ?>'
                                });
                                $confirmBtn.prop('disabled', false).html('<i class="ri-delete-bin-line me-1"></i><?= __('Xóa nhật ký'); ?>');
                            }
                        },
