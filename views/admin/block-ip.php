...
if (isset($_POST['AddIPBlock'])) {
    ...
    // Xử lý nhiều IP, mỗi dòng 1 IP
    $raw_ips = trim($_POST['ip']);
    $reason  = !empty($_POST['reason']) ? check_string($_POST['reason']) : NULL;

    // Tách IP theo từng dòng, loại bỏ khoảng trắng dư thừa và dòng rỗng
    $ipArray = array_unique(array_filter(array_map('trim', explode("\n", str_replace("\r", "", $raw_ips)))));

    $totalInsert = 0;
    foreach ($ipArray as $ipv) {
        if ($CMSNT->num_rows("SELECT * FROM `block_ip` WHERE `ip` = '$ipv' ") > 0) {
            continue; // bỏ qua IP đã tồn tại
        }
    ...
    if ($totalInsert > 0) {
        ...
        $my_text = str_replace('{domain}', $_SERVER['SERVER_NAME'], $my_text);
        $my_text = str_replace('{username}', $getUser['username'], $my_text);
        $my_text = str_replace('{action}', "Add Block IP ($totalInsert)", $my_text);
        $my_text = str_replace('{ip}', myip(), $my_text);
        $my_text = str_replace('{time}', gettime(), $my_text);
        sendMessAdmin($my_text);
        die('<script type="text/javascript">if(!alert("' . __('Thêm thành công!') . '")){location.href = "' . base_url_admin('block-ip') . '";}</script>');
    } else {
        die('<script type="text/javascript">if(!alert("' . __('Thêm thất bại hoặc IP đã tồn tại!') . '")){window.history.back().location.reload();}</script>');
    }
}
    ...
    <div class="container-fluid">
        <div class="d-md-flex d-block align-items-center justify-content-between my-4 page-header-breadcrumb">
            <div class="d-flex align-items-center gap-3">
                ...
                <?php if (checkPermission($getUser['admin'], 'edit_block_ip')): ?>
                    <button type="button" class="btn btn-outline-danger btn-sm" data-bs-toggle="modal" data-bs-target="#cleanupBlockIPModal">
                    ...
                    <div class="row mb-4">
                        ...
                        <div class="col-sm-8">
                            <div class="input-group mb-3">
                                <textarea class="form-control" id="ip" name="ip" rows="5"
                                    placeholder="<?= __('Nhập địa chỉ IP cần Block, mỗi địa chỉ một dòng'); ?>" required></textarea>
                            </div>
                        </div>
                    </div>
                    <div class="row mb-4">
...
<!-- Modal dọn dẹp block IP -->
<div class="modal fade" id="cleanupBlockIPModal" tabindex="-1" aria-labelledby="cleanupBlockIPModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="cleanupBlockIPModalLabel">
                    <i class="ri-delete-bin-line text-danger me-2"></i><?= __('Dọn dẹp danh sách IP bị chặn'); ?>
                </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <div class="alert alert-warning border-0">
                    ...
                    <?= __('Lưu ý: Thao tác này sẽ xóa vĩnh viễn các IP và không thể hoàn tác.'); ?>
                </div>
                <div class="mb-3">
                    <label for="cleanupDaysBlockIP" class="form-label fw-medium"><?= __('Xóa IP cũ hơn'); ?></label>
                    <div class="input-group">
                        <input type="number" class="form-control" id="cleanupDaysBlockIP" value="30" min="1" max="365" placeholder="30">
                        <span class="input-group-text"><?= __('ngày'); ?></span>
                    </div>
                    <div class="form-text">
                        <i class="ri-information-line me-1"></i>
                        <?= __('Ví dụ: Nhập 30 sẽ xóa tất cả IP bị chặn cũ hơn 30 ngày trở về trước.'); ?>
                    </div>
                </div>
                <div id="cleanupPreviewBlockIP" class="d-none">
                    <div class="alert alert-info-transparent border mb-0">
                        <i class="ri-file-list-3-line me-2"></i>
                        <span id="cleanupPreviewTextBlockIP"></span>
                    </div>
                </div>
            </div>
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-danger" id="confirmCleanupBtnBlockIP">
                    <i class="ri-delete-bin-line me-1"></i><?= __('Xóa IP'); ?>
                </button>
            </div>
        </div>
    </div>
...
<script>
    $(document).ready(function() {
        var cleanupModal = document.getElementById('cleanupBlockIPModal');
        var $cleanupDays = $('#cleanupDaysBlockIP');
        var $cleanupPreview = $('#cleanupPreviewBlockIP');
        var $cleanupPreviewText = $('#cleanupPreviewTextBlockIP');
        var $confirmBtn = $('#confirmCleanupBtnBlockIP');
        ...
        function updatePreview() {
            ...
            $.ajax({
                ...
                dataType: 'json',
                data: {
                    action: 'previewCleanupBlockIP',
                    days: days
                },
                success: function(resp) {
                    if (resp.status === 'success') {
                        $cleanupPreviewText.text('<?= __('Sẽ xóa'); ?> ' + resp.count + ' <?= __('IP'); ?>');
        ...
        if (cleanupModal) {
            ...
            cleanupModal.addEventListener('hidden.bs.modal', function() {
                $cleanupPreview.addClass('d-none');
                $confirmBtn.prop('disabled', false).html('<i class="ri-delete-bin-line me-1"></i><?= __('Xóa IP'); ?>');
            });
        }
        ...
        $confirmBtn.on('click', function() {
            ...
            Swal.fire({
                ...
                text: '<?= __('Bạn có chắc chắn muốn xóa tất cả IP bị chặn cũ hơn'); ?> ' + days + ' <?= __('ngày?'); ?>',
